...
This applies inside `/plan`, `/goals`, `/goal`, `/work-issue`, `/execute` and
`/cycle`, not only when someone asks "how should we split this". `/plan` writes
...
reads. `/issue new` and `/issue from-plan` follow this shape; `/pr` writes
What / Why / Test with caps enforced by `guard-bash.js`.
