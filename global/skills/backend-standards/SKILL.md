...
description: >-
  Areté backend conventions across Python, TypeScript and Elixir: API and service structure,
  auth, background jobs, CLI tools, server-side logic. Load alongside the language skill.
when_to_use: >-
  Any backend work. Triggers: api, endpoint, service, backend, auth, middleware, background
  job, queue, worker, CLI tool, server.
---
