...
name: pre-impl-audit
description: >
  ALWAYS use before writing code that changes a contract — schemas, state machines,
  content templates, rendering paths, action signatures, public APIs, YAML the app
  reads, or any field read by multiple consumers. Forces an explicit consumer/docs
  audit before edits to prevent the "I changed X but didn't notice Y also reads X"
  failure mode. Skip for: test-only changes, docs-only changes, single-line cosmetic
  fixes, or net-new code with no callers yet.
  Trigger phrases: "before I edit", "audit consumers", "contract change", "schema
  change", "state machine", "template change", "render path", "rename", "refactor",
  "change yaml", "rework", "lifecycle".
---
