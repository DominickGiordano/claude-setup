...
name: ash
description: >-
  Areté overlay for Ash: resources, actions, policies, code interfaces, AshPhoenix.Form,
  AshPostgres migrations. Corrects upstream usage_rules:ash guidance that burned us in
  production — load alongside the project's ash-framework skill, not instead of it.
when_to_use: >-
  Writing or modifying Ash code. Triggers: ash, ash resource, ash action, ash policy, ash
  changeset, code interface, Ash.read, Ash.create, Ash.update, Ash.get, upsert,
  after_transaction, ash_phoenix, ash form, multitenancy.
---
...
| File | Read when |
|---|---|
| `references/migrations-and-verification.md` | Generating an AshPostgres migration, naming an identity index, or verifying that a newly defined action has real callers |
