...
- **Not `arete-portfolio`.** That maps repos across the org. This goes deep on one.
...
- `areteos/docs/architecture.md` documented a `GateStepExecutor`. No such file exists. It
  described 9 domains; there are ~30.
- `arete-terraform-infrastructure/docs/architecture.md` listed 3 apps. There are 11 —
  it omitted `areteos` itself.
...
- **Narrow.** `lib/areteos/runs/**` over `lib/**`. Too broad → every session flags the doc
...
- **Architecture watches source. Runbook watches ops** — `.github/workflows/**`,
  `docker-compose*.yml`, `Dockerfile*`, `*.tf`, `mix.exs`, `.env.example`. A refactor under
...
```
/repo-docs — areteos (Elixir/Phoenix/Ash, 30 domains, 2621 commits since last doc)

  docs/architecture.md   REFRESH — last verified 2026-04-29, 3955 changes in lib/ since
    watches: lib/areteos/runs/**, lib/areteos/workflows/**, config/**
    sections: Overview · 30 domains (was 9) · Spine · 9 step executors (was 4, one fictional) · Decisions · Deps
    corrections: GateStepExecutor does not exist · 21 domains undocumented

  docs/runbook.md        NEW
    watches: .github/workflows/**, Dockerfile, mix.exs, docker-compose.yml
    sections: Local · Envs · Secrets (Infisical /areteos/) · Deploy · Rollback · Scheduled
...
watches: set. Next session that changes lib/areteos/runs/** will flag these.
```
