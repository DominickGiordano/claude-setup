...
description: >-
  Defining LLM tools and agentic loops in Elixir with Jido + ReqLLM (areteos): the 1-arity
  tool callback contract, per-invocation context closures, AshJido to ReqLLM bridging, model
  aliases, and the ReqLLM call shapes plus their gotchas.
when_to_use: >-
  Triggers: jido, req_llm, ReqLLM, tool, tool_call, agent loop, AgentLoop, AshJido, model
  alias, embed, generate_object, stream_text, generate_text.
---
